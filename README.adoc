...
A quicker solution would be to just install the necessary Python dependencies in a virtual environment.
If you have link:url-python-invoke[Python Invoke^] installed, you may use this command to install dependencies:
...
If you don't have link:url-python-invoke[Invoke^] installed, you can install the `pip` dependencies manually (assuming you have `pip` and `venv` installed)
